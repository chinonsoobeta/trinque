...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const t = useUiText();
  const desktopLinks = [{ href: "/", label: t("nav.discover") }, { href: "/explore?feed=following", label: t("nav.following") }, { href: "/?view=groups", label: t("nav.groups") }, { href: "/?view=saved", label: t("nav.saved") }];
  ...
  return <div className="global-app-shell">
    <header className={`app-header${authSurface ? " app-header-minimal" : ""}`}>
      <Link className="app-brand" href="/" onClick={(event) => switchRootView(event, "/")} aria-label={t("home.title")}><span>T</span><b>Trinque</b></Link>
