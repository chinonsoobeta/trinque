...
export default function ModerationPage() {
  ...
  // A message key rather than a translated string, so the queue load below does
  // not depend on `t` — that identity changes when the language store hydrates,
  // and the queue was being fetched twice because of it.
  ...
  useEffect(() => {
    ...
    void fetch("/api/moderation", { headers: authHeaders(), cache: "no-store" }).then(async (response) => {
      ...
      if (!response.ok) { setStatusKey("safety.failed"); return; }
      const payload = await response.json() as { reports?: Report[] };
      setReports(payload.reports ?? []);
    });
    return () => { active = false; };
  }, [authenticated, authHeaders, loading]);
