...
export function CommentSection({ dishId, dishOwnerId }: { dishId: string; dishOwnerId: string }) {
  ...
  // Held as a message key so the load below does not depend on `t`, whose
  // identity changes once the language store hydrates — that dependency made
  // every dish page fetch its comments twice.
  const [statusKey, setStatusKey] = useState<MessageKey | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => { let active = true; void fetch(`/api/dishes/${encodeURIComponent(dishId)}/comments`).then((response) => response.json()).then((payload: { comments?: Comment[] }) => { if (active) setComments(payload.comments ?? []); }).catch(() => setStatusKey("comments.unavailable")).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [dishId]);
  ...
  async function submit(event: FormEvent) {
    ...
    setComments((current) => [optimistic, ...current]); setBody(""); setStatusKey(null);
    try {
      ...
      setComments((current) => current.filter((comment) => comment.id !== tempId)); setBody(text); setStatusKey("comments.postFailed");
    }
  }
  ...
  async function remove(commentId: string) {
    ...
    else setStatusKey("safety.failed");
  }

  return <section className="comment-section"><h2>{t("comments.title")}</h2>{loading && <LoadingState label={t("comments.loading")} />}<form onSubmit={submit}><textarea maxLength={1000} value={body} onChange={(event) => setBody(event.target.value)} placeholder={authenticated ? t("comments.add") : t("comments.signIn")} /><button className="primary" disabled={!body.trim()}>{t("comments.post")}</button></form>{statusKey && <p role="status">{t(statusKey)}</p>}<div>{comments.map((comment) => <article key={comment.id}><b>{comment.displayName ?? t("comments.member")}</b>{comment.handle && <span> @{comment.handle}</span>}<p>{comment.body}</p><small>{new Intl.DateTimeFormat(language, { dateStyle: "medium", timeStyle: "short" }).format(new Date(comment.createdAt))}</small><SafetyActions targetType="comment" targetId={comment.id} userId={comment.userId} />{authenticated && (identity?.id === comment.userId || identity?.id === dishOwnerId) && <button className="text-button danger" onClick={() => void remove(comment.id)}>{t("safety.removeComment")}</button>}</article>)}</div></section>;
}
