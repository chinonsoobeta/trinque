...
Phase 5 replaces the prototype invite display with membership-authorized group plans backed by Turso.
...
New plans are built from nearby published dish records in Turso and, when configured, Google restaurant-level alternatives. Demo restaurants are never substituted into a live plan. Restaurant-level alternatives remain ineligible when price, dietary fit, allergens, or menu availability cannot be supported; the UI says that no matching dish is claimed.
...
Without `GOOGLE_PLACES_API_KEY`, group creation still uses eligible nearby published Turso dishes. Provider health remains unavailable and no seeded Vancouver or other demo results are inserted. A real three-session route smoke test and real provider candidate check require deployed Turso/Places configuration.
