...
export function ProfileEditor({ profile }: { profile: PublicProfile }) {
  ...
  if (!editing) return <button className="secondary" onClick={() => setEditing(true)}>{t("profile.edit")}</button>;

  async function uploadAvatar(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) return;
    setBusy(true); setStatus(t("onboarding.saving"));
    try {
      ...
      if (!response.ok || !payload.avatarUrl) { setStatus(t("onboarding.photoFailed")); return; }
      setAvatarUrl(payload.avatarUrl); setStatus(t("safety.done"));
  ...
  async function removeAvatar() {
    ...
    try {
      ...
      setAvatarUrl(null); setStatus(t("safety.done"));
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !payload.profile) { setStatus(t("onboarding.saveFailed")); return; }
  ...
  return <form className="profile-editor" onSubmit={submit}>
    ...
    <label>{t("onboarding.name")}<input maxLength={80} value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></label>
    <label>{t("onboarding.username")}<input maxLength={30} value={handle} onChange={(event) => setHandle(event.target.value.toLowerCase())} /></label>
    <label>{t("profile.bio")}<textarea maxLength={500} value={bio} onChange={(event) => setBio(event.target.value)} /></label>
    <label>{t("settings.location")}<input maxLength={100} value={location} onChange={(event) => setLocation(event.target.value)} /></label>
    <div><button className="primary" disabled={busy}>{t("onboarding.save")}</button><button type="button" className="text-button" onClick={() => setEditing(false)}>{t("safety.cancel")}</button></div>
