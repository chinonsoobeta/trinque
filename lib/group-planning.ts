export type GroupConstraints = { budgetMax: number; maxDistanceKm: number; vegetarianRequired: number; allergies: string[] };
export type GroupCandidateSource = { candidateId: string; name: string; restaurant: string; neighborhood: string; distanceKm: number; priceAmount: number | null; currencyCode: string; image?: string | null; dietaryCaveat: string; kind: "published_dish" | "provider_restaurant"; restaurantId?: string | null; providerPlaceId?: string | null; provenance: string; verificationStatus: string; currentAvailabilityConfirmed: boolean };
...
export function rankGroupCandidates(candidates: GroupCandidateSource[], constraints: GroupConstraints, locale = "en-CA"): RankedGroupCandidate[] {
  return candidates.map((candidate) => {
    ...
    const supportsVegetarian = /vegetarian|plant-based|végétar|vegetariano|vegano/.test(candidate.dietaryCaveat.toLowerCase());
