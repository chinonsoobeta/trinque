...
function groupCandidateCopy(t: Translator, candidate: GroupCandidate) {
  const explanation = candidate.explanation === "eligible" ? t("group.fitEligible") : `${t("group.fitIneligible")} ${candidate.conflicts.map((conflict) => groupConflictLabel(t, conflict)).join("; ")}`.trim();
...
export default function Home() {
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        {view === "groups" ? (
          <GroupPlanner guestToken={guestToken} flash={flash} t={t} location={location} language={language} track={trackAnalytics} onRequestLocation={() => setSettingsOpen(true)} />
        ) : (
          <>
...
function GroupPlanner({ guestToken, flash, t, location, language, track, onRequestLocation }: { guestToken: string | null; flash: (text: string) => void; t: Translator; location: NormalizedLocation | null; language: UiLanguage; track: (event: AnalyticsEvent, details?: { mode?: "live" | "demo"; outcome?: string; durationMs?: number }) => void; onRequestLocation: () => void }) {
  const { authenticated } = useAuth();
  ...
  return <section className="group-page">
    ...
    <div className="planner"><aside className="constraints"><span className="kicker">{new Intl.DateTimeFormat(language, { weekday: "short", hour: "numeric", minute: "2-digit", timeZone: group.timeZone ?? "UTC" }).format(new Date(group.eventTime))}</span><h2>{group.name}</h2>{[["◎", group.locality ?? group.neighborhood, new Intl.NumberFormat(language, { style: "unit", unit: group.distanceUnit === "imperial" ? "mile" : "kilometer", unitDisplay: "short", maximumFractionDigits: 1 }).format(group.distanceUnit === "imperial" ? group.maxDistanceKm * .621371 : group.maxDistanceKm)], ["$", new Intl.NumberFormat(group.locale ?? language, { style: "currency", currency: group.currencyCode ?? "CAD", maximumFractionDigits: 0 }).format(group.budgetMax), t("group.budget")], ["!", group.allergies.join(", ") || "—", t("group.allergies")], ["♧", group.vegetarianRequired.toString(), t("group.vegetarian")]].map(([icon, title, note], index) => <div className={index === 2 && group.allergies.length ? "constraint warning" : "constraint"} key={`${icon}-${title}`}><span>{icon}</span><div><b>{title}</b><small>{note}</small></div></div>)}{group.viewerRole === "owner" && !group.inviteRevokedAt && <><button className="secondary full" onClick={() => { void navigator.clipboard?.writeText(`${window.location.origin}/?join=${group.inviteCode}`); flash(t("group.inviteCopied")); }}>{t("group.copyInvite")}</button><button className="text-button full" onClick={() => void groupAction("invite/revoke")}>{t("group.revokeInvite")}</button></>}</aside>
      <div className="shortlist"><div className="section-heading"><div><span className="kicker">{t("group.ranking")}</span><h2>{group.status === "voting" ? t("group.bestFits") : t("group.finalPlan")}</h2></div></div>
        {group.candidates.length === 0 && <p className="empty-tier">{t("group.noLiveCandidates")}</p>}{group.candidates.slice(0, 5).map((candidate, index) => { const copy = groupCandidateCopy(t, candidate); const distance = new Intl.NumberFormat(language, { style: "unit", unit: group.distanceUnit === "imperial" ? "mile" : "kilometer", unitDisplay: "short", maximumFractionDigits: 1 }).format(group.distanceUnit === "imperial" ? candidate.distanceKm * .621371 : candidate.distanceKm); return <article className={`${candidate.eligible && index === 0 ? "vote-card winner" : "vote-card"} ${candidate.eligible ? "" : "ineligible"}`} key={candidate.candidateId}><div className="vote-image" style={candidate.image ? { backgroundImage: `url(${candidate.image})` } : undefined}><span>{candidate.eligible ? `#${index + 1}` : "!"}</span></div><div className="vote-copy"><div><span>{candidate.eligible ? t("group.groupFit", { score: candidate.score }) : t("group.hardConflict")}</span><h3>{candidate.restaurant}</h3><p>{candidate.name} · {candidate.price} · {distance}</p><small>{copy.explanation}</small><small>{candidate.kind === "provider_restaurant" ? t("match.restaurantReason") : `${t(`provenance.${candidate.provenance ?? "community_submitted"}` as MessageKey)} · ${t(`verification.${candidate.verificationStatus ?? "unverified"}` as MessageKey)}`} · {candidate.currentAvailabilityConfirmed ? t("availability.confirmed") : t("availability.unknown")}</small><small>{copy.dietaryCaveat}</small></div><button disabled={busy || !candidate.eligible || group.status === "finalized"} onClick={() => void groupAction("vote", { candidateId: candidate.candidateId })}>▲ <b>{group.votes[candidate.candidateId] ?? 0}</b></button></div></article>; })}
