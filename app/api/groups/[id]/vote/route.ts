...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [group] = await db.select().from(groups).where(and(eq(groups.id, id), eq(groups.ownerId, identity.id))).limit(1);
  if (!group || group.status !== "voting") return Response.json({ error: "Open group plan not found." }, { status: 404 });
  ...
  return Response.json({ group: await groupSnapshot(id) });
}
