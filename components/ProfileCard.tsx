...
export function ProfileCard({ profile, counts, action }: { profile: PublicProfile; counts: { followers: number; following: number; dishes: number }; action?: React.ReactNode }) {
  return <section className="profile-card">
    ...
    <div><h1>{profile.displayName}</h1><p>@{profile.handle}</p>{profile.bio && <p>{profile.bio}</p>}{profile.location && <p>{profile.location}</p>}<p><b>{counts.followers}</b> followers · <b>{counts.following}</b> following · <b>{counts.dishes}</b> dishes</p></div>
    {action}
  </section>;
}
