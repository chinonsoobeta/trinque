...
export async function GET(request: Request) {
  try { const identity = await requireOnboardedIdentity(request); const db = await getDb(); const reports = await db.select().from(contentReports).where(eq(contentReports.reporterId, identity.id)).orderBy(desc(contentReports.createdAt)).limit(100); return Response.json({ reports }); }
...
export async function POST(request: Request) {
  try {
    const identity = await requireOnboardedIdentity(request); await enforceUsageBudget("report", identity.id);
