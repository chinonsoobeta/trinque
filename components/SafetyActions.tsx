...
export function SafetyActions({ targetType, targetId, userId, allowHide = false }: Props) {
  ...
  return <div className="modal-actions safety-actions" aria-label={t("safety.title")}>
    ...
    {userId && <><button className="text-button" disabled={busy} onClick={() => void act("/api/safety", { action: "mute", targetId: userId })}>{t("safety.muteUser")}</button><button className="text-button danger" disabled={busy} onClick={block}>{t("safety.blockUser")}</button></>}
