...
export function DishDetailView({ dish, related, likeCount, commentCount }: { dish: DishDetail; related: RelatedDish[]; likeCount: number; commentCount: number }) {
  ...
  return <PageContainer className="dish-detail-page"><article className="dish-detail-article">
    <div className={`dish-detail-image${dish.imageKey ? "" : " is-empty"}`}>{dish.imageKey ? <Image src={`/api/media/${dish.imageKey}`} alt={dish.name} width={1200} height={900} sizes="(max-width: 768px) 100vw, 50vw" /> : <div><Icon name="sparkle" size={30} className="empty-mark" /><p>{t("dish.noRetainedPhoto")}</p></div>}</div>
    <div className="dish-detail-main"><span className="kicker">{dish.cuisine}</span><h1>{dish.name}</h1><p className="dish-restaurant">{dish.restaurantName ?? t("dish.noRestaurant")}{dish.restaurantLocality ? <span> · {dish.restaurantLocality}</span> : null}</p><p className="dish-description">{dish.description}</p>
      ...
      <div className="dish-detail-actions"><LikeButton dishId={dish.id} initialCount={likeCount} /><a className="secondary compact-action button-link" href="#comments"><Icon name="comment" size={16} />{commentCount} {t("comments.title")}</a><DishShareButton title={dish.name} /></div>
    ...
    {related.length > 0 && <section className="related-dishes"><div className="section-heading"><div><span className="kicker">{t("nav.explore")}</span><h2>{t("dish.related")}</h2></div></div><div className="related-dish-grid">{related.map((item) => <a className="related-dish-card" href={`/dishes/${item.id}`} key={item.id}>{item.imageKey ? <Image src={`/api/media/${item.imageKey}`} alt="" width={480} height={360} sizes="(max-width: 768px) 100vw, 33vw" /> : <div className="related-placeholder"><Icon name="sparkle" size={22} /></div>}<div><h3>{item.name}</h3><p>{item.description}</p></div></a>)}</div></section>}
  </PageContainer>;
}
