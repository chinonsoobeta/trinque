...
Live matching reads reviewed, published dish records from Turso and normalized restaurant alternatives from the configured Places provider. The old deterministic catalog is exported only as `demoNearbyCatalog` and is not reachable from the live publication path.
...
Provider failures are returned separately from Turso dish results. Missing credentials or a live provider failure never causes seeded restaurants to appear in live tiers.
