...
export function AuthControls() {
  ...
  if (authenticated) return <div className="setting-block account-block"><h2>{t("auth.account")}</h2><p className="privacy-note">{t("auth.signedIn", { name: identity?.displayName ?? "" })}{identity?.email ? ` · ${identity.email}` : ""}</p><Link className="secondary full button-link" href="/account">{t("auth.openAccount")}</Link><AccountPrivacyActions /><button className="text-button full" onClick={() => void signOut()}>{t("auth.signOut")}</button></div>;
  return <div className="setting-block account-block"><h2>{t("auth.account")}</h2><p className="privacy-note">{t("auth.signInHelp")}</p><Link className="secondary full button-link" href="/auth/login?next=/">{t("auth.signIn")}</Link></div>;
}
